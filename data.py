...
class Dataset(object):
    ...
    def prepare_train_test(self):
        ...
        return train[:, :self.timesteps], train[:, self.timesteps:self.timesteps + steps_ahead], \
            test[:, :self.timesteps], test[:,
                                           self.timesteps:self.timesteps + steps_ahead]
    ...
    def _load_file(self, file_path):
        ...
        if ext == '.txt':
            with open(file_path, 'r') as f:
                data = f.readlines()
                self.min, self.max = list(
                    map(lambda x: float(x), data[0].split(' ')))
            ...
            data = raw_data.apply(lambda x: (
                x - raw_data.min()) / (raw_data.max() - raw_data.min()))
        return data
    ...
    def _preprocess(self):
        ...
        print('total: {}, train: {}, test: {}.'.format(
            len(raw_data), num_train, len(raw_data) - num_train))
        train, test = [], []
        ...
        for i, path in enumerate([self.train_path, self.test_path]):
            with open(path, 'w') as f:
                ...
                for sample in content[i]:
                    f.write(
                        ' '.join(map(lambda x: '{:.6f}'.format(x), sample)) + '\n')
